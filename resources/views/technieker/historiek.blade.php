...
<div class="mb-10 flex flex-col md:flex-row justify-between md:items-end gap-6">
    ...
    <div class="w-full md:w-auto">
        <a href="{{ route('materiaal.bestellen') }}" class="w-full md:w-auto h-12 bg-white text-[#005b96] border border-blue-100 px-6 rounded-xl font-black text-sm shadow-sm flex items-center justify-center gap-2 active:scale-95 transition-all hover:bg-blue-50">
...
<div class="grid grid-cols-1 gap-4">
    @forelse($bestellingen as $order)
        ...
        <div class="bg-white rounded-[2rem] border border-slate-200 shadow-[0_2px_15px_rgba(0,0,0,0.02)] p-5 md:p-6 flex flex-col md:flex-row gap-5 items-start md:items-center group hover:border-[#005b96]/40 transition-all duration-300">
            ...
            <div class="w-16 h-16 shrink-0 rounded-2xl bg-gradient-to-br {{ $gradient }} flex items-center justify-center shadow-md group-hover:scale-105 transition-transform duration-500">
                @if(isset($order->materiaal->foto) && $order->materiaal->foto)
                    <img src="{{ asset('storage/' . $order->materiaal->foto) }}" class="w-full h-full object-cover rounded-2xl">
                @else
                    <svg class="w-8 h-8 text-white drop-shadow-sm" fill="none" stroke="currentColor" viewBox="0 0 24 24">{!! $iconPath !!}</svg>
                @endif
            </div>

            <div class="flex-grow w-full min-w-0">
                <div class="flex flex-wrap items-center gap-3 mb-2">
                    <span class="text-[10px] font-black tracking-widest text-[#005b96] bg-blue-50 border border-blue-100 px-2 py-0.5 rounded-md uppercase">{{ $order->materiaal->artikelnummer ?? 'N/A' }}</span>
                    <span class="text-[10px] font-bold text-slate-400 flex items-center gap-1">
                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                        {{ $order->created_at->format('d M Y - H:i') }}
                    </span>
                </div>
                <h3 class="text-lg font-black text-slate-800 leading-tight group-hover:text-[#005b96] transition-colors">{{ $order->materiaal->omschrijving ?? 'Onbekend materiaal' }}</h3>
            </div>
