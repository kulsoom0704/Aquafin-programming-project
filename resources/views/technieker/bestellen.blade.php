...
@section('content')
    ...
    <div class="mb-8 flex flex-col lg:flex-row justify-between lg:items-end gap-4">
        <div>
            <span class="text-[10px] md:text-xs font-black tracking-[0.2em] text-[#005b96] uppercase mb-1 block">Aquafin
                Logistiek — Depot {{ session('depot', 'Antwerpen') }}</span>
        ...
        <div class="flex flex-col sm:flex-row gap-3 relative z-30 w-full lg:w-auto">
            <div class="w-full sm:w-80 relative">
                ...
                <input type="text" id="searchInput" placeholder="Snel zoeken op naam of ref..." autocomplete="off"
                    class="w-full pl-11 pr-4 py-3 bg-white border border-slate-200 rounded-2xl focus:outline-none focus:border-[#005b96] focus:ring-4 focus:ring-blue-500/5 transition-all text-sm font-medium">
                <div id="searchSuggestions"
                    class="absolute z-[60] w-full bg-white border border-slate-100 shadow-2xl rounded-2xl mt-2 hidden max-h-60 overflow-y-auto custom-scrollbar p-1">
                </div>
            </div>

            <button onclick="toggleCart()"
                class="w-full sm:w-auto bg-[#005b96] hover:bg-[#004a7c] text-white px-6 py-3 rounded-2xl font-bold shadow-md shadow-blue-500/10 active:scale-95 transition-all flex items-center justify-center gap-2">
                ...
                <span id="cartCount"
                    class="bg-cyan-400 text-[#001e33] text-[10px] px-2 py-0.5 rounded-full font-black ml-1 scale-0 transition-transform">0</span>
            </button>
        </div>
    </div>
    ...
    @if(isset($weer) && $weer['is_beschikbaar'])
        <div id="weatherSection" class="mb-8">
            <div
                class="bg-gradient-to-br from-[#001e33] to-[#00365c] rounded-3xl p-5 md:p-6 relative shadow-lg border border-white/5 transition-all">
                ...
                <div class="relative z-10">
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 cursor-pointer group"
                        onclick="toggleWeer()">
                        <div>
                            <span
                                class="px-2.5 py-0.5 rounded-md bg-cyan-500/10 text-cyan-300 text-[9px] font-black tracking-widest uppercase border border-cyan-400/10 mb-1 inline-block transition-colors group-hover:bg-cyan-500/20">Weersadvies</span>
                            <div class="flex items-center gap-3">
                                <h2 class="text-xl font-black text-white tracking-tight">Geadviseerde uitrusting voor vandaag
                                </h2>
                                <div
                                    class="w-7 h-7 rounded-full bg-white/10 flex items-center justify-center text-white group-hover:bg-cyan-500 group-hover:text-[#001e33] transition-colors shadow-sm">
                                    <svg id="weerChevron" class="w-4 h-4 transition-transform duration-300" fill="none"
                        ...
                        <div class="flex items-center gap-3 bg-white/5 px-4 py-2 rounded-xl border border-white/5 w-full md:w-auto"
                            ...
                            <span
                                class="text-xs font-bold {{ $weer['gevaar'] == 'Kritiek' ? 'text-rose-400' : ($weer['gevaar'] == 'Gemiddeld' ? 'text-amber-400' : 'text-emerald-400') }}">
                    ...
                    <div id="weerContent" class="transition-all duration-500 overflow-hidden"
                        style="max-height: 1000px; opacity: 1; margin-top: 1.25rem;">
                        @if($aanbevolenMaterialen->count() > 0)
                            <!-- Verticale scroll voor de advieskaarten -->
                            <div
                                class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 max-h-[350px] overflow-y-auto custom-scrollbar pr-2 pb-2">
                                @foreach($aanbevolenMaterialen as $item)
                                    <div class="bg-white/5 border border-white/5 rounded-2xl p-5 flex flex-col justify-between hover:bg-white/10 transition-all cursor-default"
                                        onclick="event.stopPropagation()">
                                        <div>
                                            <div class="flex justify-between items-center mb-3">
                                                <span
                                                    class="text-[10px] font-black text-cyan-300 tracking-wider bg-slate-900/50 px-2.5 py-1 rounded-lg">{{ $item->artikelnummer }}</span>
                                                <div
                                                    class="w-2 h-2 rounded-full {{ $item->beschikbaar > 0 ? 'bg-emerald-400' : 'bg-rose-400' }}">
                                                </div>
                                            </div>
                                        ...
                                        <div class="flex items-center gap-2 mt-auto">
                                            <input type="number" id="qty-rec-{{ $item->id }}" min="1"
                                                max="{{ $item->beschikbaar > 0 ? $item->beschikbaar : 1 }}" value="1" {{ $item->beschikbaar == 0 ? 'disabled' : '' }}
                                                class="w-12 h-10 bg-slate-900/60 border border-white/10 text-white rounded-xl text-center font-bold text-xs focus:outline-none">
                                            <button
                                                onclick="addToCart({{ $item->id }}, '{{ addslashes($item->omschrijving) }}', '{{ $item->artikelnummer }}', {{ $item->beschikbaar }}, 'rec'); event.stopPropagation();"
                                                {{ $item->beschikbaar == 0 ? 'disabled' : '' }}
                                                class="flex-grow h-10 bg-cyan-400 text-[#001e33] font-black rounded-xl text-xs active:scale-95 transition-transform flex items-center justify-center shadow-lg shadow-cyan-500/20">
                                                Toevoegen
                                            </button>
                                        </div>
                                    </div>
    ...
    <div class="flex gap-2 overflow-x-auto pb-4 w-full hide-scrollbar border-b border-slate-200 mb-6">
        <button
            class="cat-btn active bg-slate-900 text-white px-5 py-2.5 rounded-xl font-bold text-xs tracking-wide whitespace-nowrap shadow-sm shrink-0"
            data-prefix="ALL">Alles</button>
        <button
            class="cat-btn bg-white text-slate-600 border border-slate-200 px-5 py-2.5 rounded-xl font-bold text-xs tracking-wide whitespace-nowrap shrink-0"
            data-prefix="BEV">Bevestiging</button>
        <button
            class="cat-btn bg-white text-slate-600 border border-slate-200 px-5 py-2.5 rounded-xl font-bold text-xs tracking-wide whitespace-nowrap shrink-0"
            data-prefix="PBM">Veiligheid</button>
        <button
            class="cat-btn bg-white text-slate-600 border border-slate-200 px-5 py-2.5 rounded-xl font-bold text-xs tracking-wide whitespace-nowrap shrink-0"
            data-prefix="GER">Gereedschap</button>
        <button id="btnFavFilter"
            class="bg-white text-rose-500 border border-rose-100 px-5 py-2.5 rounded-xl font-bold text-xs flex items-center gap-2 shrink-0 ml-auto">
            ...
            <div class="product-card bg-white p-5 rounded-[1.5rem] border border-slate-200 shadow-sm flex flex-col justify-between group hover:border-[#005b96]/40 hover:shadow-xl transition-all duration-300"
                ...
                <div class="flex items-center justify-between pt-4 border-t border-slate-100">
                    ...
                    <div class="flex items-center gap-2">
                        <input type="number" id="qty-main-{{ $item->id }}" min="1"
                            max="{{ $item->beschikbaar > 0 ? $item->beschikbaar : 1 }}" value="1" {{ $item->beschikbaar == 0 ? 'disabled' : '' }}
                            class="w-14 h-11 bg-slate-50 border border-slate-200 text-slate-900 rounded-xl text-center font-black text-sm focus:outline-none focus:border-[#005b96] focus:ring-2 focus:ring-blue-500/20 transition-all">
                        <button
                            onclick="addToCart({{ $item->id }}, '{{ addslashes($item->omschrijving) }}', '{{ $item->artikelnummer }}', {{ $item->beschikbaar }}, 'main')"
                            {{ $item->beschikbaar == 0 ? 'disabled' : '' }}
                            class="w-11 h-11 bg-[#005b96] text-white disabled:bg-slate-100 disabled:text-slate-400 hover:bg-[#004a7c] rounded-xl flex items-center justify-center active:scale-95 transition-all shadow-md shadow-blue-900/10">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5"
    ...
    <div id="noResults"
        class="hidden py-16 text-center flex-col items-center justify-center bg-white rounded-2xl border border-dashed border-slate-200 mt-6 shadow-sm">
    ...
    <div id="cartSidebar"
        class="fixed inset-y-0 right-0 w-full sm:w-[400px] bg-slate-50 shadow-[-10px_0_40px_rgba(0,0,0,0.1)] transform translate-x-full transition-transform duration-300 z-[100] flex flex-col border-l border-slate-200">
        <div class="p-5 bg-white flex justify-between items-center border-b border-slate-100 pt-safe">
            ...
            <button onclick="toggleCart()"
                class="w-9 h-9 flex items-center justify-center bg-slate-100 text-slate-500 rounded-xl hover:bg-slate-200 transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="absolute bottom-0 inset-x-0 p-4 bg-white border-t border-slate-100 pb-safe">
            <form action="{{ route('materiaal.bestellen.store') }}" method="POST" id="checkoutForm">
                ...
                <button type="submit" id="btnCheckout"
                    class="w-full h-13 bg-[#005b96] hover:bg-[#004a7c] text-white rounded-xl font-black shadow-lg disabled:opacity-50 flex items-center justify-center gap-2 active:scale-95 transition-transform">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div id="cartOverlay" onclick="toggleCart()"
        class="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-40 hidden opacity-0 transition-opacity duration-300"></div>
