...
<div class="flex flex-col md:flex-row justify-between md:items-end gap-4 mb-8">
    ...
    <div class="flex gap-4 w-full md:w-auto mt-2 md:mt-0">
        <div class="bg-white border border-slate-200 rounded-2xl px-5 py-3 shadow-[0_2px_10px_rgba(0,0,0,0.02)] flex items-center gap-4 flex-grow md:flex-grow-0">
            ...
            <div>
                <div class="text-2xl font-black text-slate-800 leading-none">{{ $meldingen->count() }}</div>
        ...
        <div class="bg-white rounded-3xl border border-slate-200 shadow-[0_2px_15px_rgba(0,0,0,0.02)] p-5 md:p-6 flex flex-col md:flex-row gap-5 items-start md:items-center group hover:border-[#005b96]/40 hover:shadow-lg transition-all duration-300">
            ...
            <div class="flex-grow w-full md:w-auto min-w-0">
                <div class="flex flex-wrap items-center gap-2 mb-2">
                    <span class="text-[9px] font-black tracking-widest uppercase text-slate-400 bg-slate-50 border border-slate-200 px-2 py-0.5 rounded-md">{{ $taak->locatie ?? 'Locatie Onbekend' }}</span>
                ...
                <h3 class="text-lg font-black text-slate-800 leading-snug group-hover:text-[#005b96] transition-colors">{{ $taak->naam }}</h3>
            ...
            <div class="flex flex-row items-center gap-2.5 w-full md:w-auto shrink-0 md:pl-6 md:border-l border-slate-100 mt-3 md:mt-0">
                <form action="{{ route('installatie.valideren', $taak->id) }}" method="POST" class="w-1/2 md:w-auto">
                    @csrf
                ...
                <a href="{{ route('installatie.show', $taak->id) }}" class="w-1/2 md:w-auto px-5 h-12 bg-slate-900 text-white rounded-xl font-bold text-xs md:text-sm text-center active:scale-95 transition-all flex items-center justify-center hover:bg-slate-800 shadow-md">
                    Logboek openen
                </a>
            </div>
        </div>
