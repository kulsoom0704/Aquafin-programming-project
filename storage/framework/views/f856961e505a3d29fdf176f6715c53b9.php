...
<head>
    ...
    <link rel="icon" type="image/png" href="<?php echo e(asset('images/image_980846.png')); ?>">
...
<body class="bg-animated text-slate-800 antialiased h-[100dvh] flex overflow-hidden pb-16 md:pb-0">
    ...
    <aside class="hidden md:flex flex-col w-72 bg-[#001e33] h-full z-40 relative shadow-[4px_0_30px_rgba(0,0,0,0.15)]">
        ...
        <div class="px-4 py-6 flex-grow">
            <nav class="space-y-1.5">
                <a href="<?php echo e(route('materiaal.bestellen')); ?>" class="flex items-center gap-3.5 px-4 py-3.5 rounded-xl font-bold text-sm transition-all <?php echo e(request()->routeIs('materiaal.bestellen') ? 'bg-[#005b96] text-white shadow-lg shadow-blue-500/20' : 'text-slate-400 hover:bg-white/5 hover:text-white'); ?>">
                    <svg class="w-5 h-5 <?php echo e(request()->routeIs('materiaal.bestellen') ? 'text-cyan-300' : 'text-slate-400 group-hover:text-white'); ?>" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"></path></svg>
                    Webshop
                </a>
                <a href="<?php echo e(route('technieker.historiek')); ?>" class="flex items-center gap-3.5 px-4 py-3.5 rounded-xl font-bold text-sm transition-all <?php echo e(request()->routeIs('technieker.historiek') ? 'bg-[#005b96] text-white shadow-lg shadow-blue-500/20' : 'text-slate-400 hover:bg-white/5 hover:text-white'); ?>">
                ...
                <a href="<?php echo e(route('technieker.meldingen')); ?>" class="flex items-center gap-3.5 px-4 py-3.5 rounded-xl font-bold text-sm transition-all <?php echo e(request()->routeIs('technieker.meldingen') ? 'bg-[#005b96] text-white shadow-lg shadow-blue-500/20' : 'text-slate-400 hover:bg-white/5 hover:text-white'); ?>">
        ...
        <div class="p-4 border-t border-white/5">
            <div class="flex items-center gap-3 bg-white/5 p-3 rounded-xl border border-white/5">
                <img class="w-10 h-10 rounded-lg object-cover shadow-inner" src="https://ui-avatars.com/api/?name=<?php echo e(urlencode(session('naam', 'T'))); ?>&background=005b96&color=fff&bold=true" alt="Avatar">
                <div class="flex-grow min-w-0">
                    <div class="text-xs font-black text-white truncate leading-tight"><?php echo e(session('naam', 'Technieker')); ?></div>
                    <a href="<?php echo e(route('logout')); ?>" class="text-[9px] font-bold text-rose-400 hover:text-rose-300 uppercase tracking-widest transition-colors flex items-center gap-1 mt-1 group">
                        Uitloggen
                    </a>
                </div>
            </div>
    ...
    <div class="md:hidden fixed top-0 inset-x-0 h-16 bg-[#001e33] border-b border-white/5 z-40 flex items-center justify-between px-4 shadow-md">
        ...
        <a href="<?php echo e(route('logout')); ?>" class="w-9 h-9 flex items-center justify-center bg-rose-500/10 text-rose-400 rounded-xl border border-rose-500/20 active:bg-rose-500/20 transition-colors">
    ...
    <div class="fixed bottom-20 right-4 md:bottom-6 md:right-6 z-50">
        <button onclick="toggleSupportTicket()" class="w-14 h-14 bg-gradient-to-r from-[#005b96] to-cyan-500 rounded-2xl flex items-center justify-center text-white shadow-2xl hover:scale-105 active:scale-95 transition-all relative border border-cyan-400/20 group">
            ...
            <?php if(isset($aantalOngelezen) && $aantalOngelezen > 0): ?>
    ...
    <div id="supportTicketWindow" class="fixed bottom-36 right-4 md:bottom-24 md:right-6 w-[calc(100vw-32px)] sm:w-96 bg-white rounded-3xl shadow-2xl border border-slate-200 z-50 translate-y-10 opacity-0 pointer-events-none transition-all duration-300 flex flex-col overflow-hidden h-[500px]">
        ...
        <?php if(isset($actieveChat) && $actieveChat): ?>
                    ...
                    <?php else: ?>
                        <div class="flex justify-start">
                            <div class="bg-white border border-slate-200 text-slate-700 p-3 rounded-2xl rounded-tl-sm max-w-[85%] shadow-sm">
                                ...
                                <p class="text-xs font-medium"><?php echo e($msg->bericht); ?></p>
                                <span class="text-[9px] text-slate-400 mt-1 block text-left"><?php echo e($msg->created_at->format('H:i')); ?></span>
                            </div>
                        </div>
                    <?php endif; ?>
            ...
            <?php if($actieveChat->status !== 'gesloten'): ?>
                <div class="p-3 bg-white border-t border-slate-100 shrink-0">
                    <form action="<?php echo e(route('chat.reply', $actieveChat->id)); ?>" method="POST" class="flex items-end gap-2">
                        <?php echo csrf_field(); ?>
    ...
    <div class="md:hidden fixed bottom-0 inset-x-0 bg-[#001e33] border-t border-white/5 pb-safe z-40 shadow-[0_-8px_30px_rgba(0,0,0,0.3)]">
        <div class="flex justify-around items-center h-16">
            ...
            <a href="<?php echo e(route('technieker.historiek')); ?>" class="flex flex-col items-center justify-center w-full h-full space-y-1 <?php echo e(request()->routeIs('technieker.historiek') ? 'text-cyan-400' : 'text-slate-400'); ?>">
            ...
            <a href="<?php echo e(route('technieker.meldingen')); ?>" class="flex flex-col items-center justify-center w-full h-full space-y-1 <?php echo e(request()->routeIs('technieker.meldingen') ? 'text-cyan-400' : 'text-slate-400'); ?>">
