...
<div class="max-w-7xl mx-auto relative pb-24 px-4 sm:px-6 lg:px-8 mt-6">
                ...
                <div class="relative z-10">
                    <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-6">
                        ...
                        <div class="flex items-center gap-4 bg-white/5 border border-white/10 px-5 py-3 rounded-2xl backdrop-blur-sm">
                            ...
                            <div>
                                <div class="text-[10px] uppercase tracking-widest text-cyan-100/70 font-bold mb-0.5">Brussel - <?php echo e($weer['temp']); ?>°C</div>
                                <div class="text-sm font-extrabold <?php echo e($weer['gevaar'] == 'Kritiek' ? 'text-rose-400' : ($weer['gevaar'] == 'Gemiddeld' ? 'text-amber-400' : 'text-emerald-400')); ?>">
                                ...
                                <div class="bg-white/10 border border-white/10 rounded-xl p-4 flex flex-col justify-between hover:bg-white/15 transition-all duration-300">
                                    <div>
                                        <div class="flex justify-between items-start mb-2">
                                            <div class="text-[10px] font-black text-cyan-300 tracking-wider bg-[#002a4a] px-2 py-1 rounded"><?php echo e($item->artikelnummer); ?></div>
                                        </div>
                                        <h3 class="text-sm font-bold text-white leading-tight mb-4"><?php echo e($item->omschrijving); ?></h3>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <input type="number" id="qty-rec-<?php echo e($item->id); ?>" min="1" max="<?php echo e($item->beschikbaar > 0 ? $item->beschikbaar : 1); ?>" value="1" <?php echo e($item->beschikbaar == 0 ? 'disabled' : ''); ?> class="w-14 h-9 bg-black/40 border border-white/20 text-white rounded-lg focus:outline-none focus:border-cyan-400 text-center font-bold text-sm">
                                        <button onclick="addToCart(<?php echo e($item->id); ?>, '<?php echo e(addslashes($item->omschrijving)); ?>', '<?php echo e($item->artikelnummer); ?>', <?php echo e($item->beschikbaar); ?>, 'rec')" <?php echo e($item->beschikbaar == 0 ? 'disabled' : ''); ?> class="flex-grow h-9 bg-cyan-500 hover:bg-cyan-400 disabled:bg-white/10 disabled:text-white/30 text-[#002a4a] font-bold rounded-lg transition-all shadow-md active:scale-95 text-xs">
                                            Toevoegen
                                        </button>
                                    </div>
                                </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-3 gap-4" id="productGrid">
        <?php $__currentLoopData = $materialen; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <div class="product-card bg-white p-3 rounded-2xl border border-slate-200 shadow-[0_2px_8px_-4px_rgba(0,0,0,0.05)] hover:border-blue-300 hover:shadow-[0_8px_20px_-4px_rgba(0,91,150,0.1)] transition-all duration-200 flex flex-row items-center gap-4 group" data-id="<?php echo e($item->id); ?>" data-ref="<?php echo e($item->artikelnummer); ?>" data-item-ref="<?php echo e(strtoupper($item->artikelnummer)); ?>">
                
                <!-- Vignette (Vibrante) -->
                <div class="w-20 h-20 shrink-0 rounded-xl relative overflow-hidden flex items-center justify-center">
                    <?php if($item->foto): ?>
                        <img src="<?php echo e(asset('storage/' . $item->foto)); ?>" alt="<?php echo e($item->omschrijving); ?>" class="w-full h-full object-cover">
                    <?php else: ?>
                        <?php
                        ...
                        ?>
                        <div class="w-full h-full bg-gradient-to-br <?php echo e($gradient); ?> flex items-center justify-center">
                    ...
                    <button class="btn-favorite absolute top-1 left-1 p-1.5 rounded-full bg-black/20 text-white/50 hover:text-rose-400 transition-colors" onclick="toggleFavorite(<?php echo e($item->id); ?>, this)">
                ...
                <div class="flex-grow flex flex-col justify-center py-1">
                    <div class="flex items-center gap-2 mb-1">
                        <span class="text-[10px] font-black tracking-widest text-slate-400"><?php echo e($item->artikelnummer); ?></span>
                        <?php if($item->beschikbaar > 0): ?>
                            <span class="text-[9px] font-bold text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded"><?php echo e($item->beschikbaar); ?> op voorraad</span>
                        <?php else: ?>
                            <span class="text-[9px] font-bold text-rose-600 bg-rose-50 px-1.5 py-0.5 rounded">Uitgeput</span>
                        <?php endif; ?>
                    </div>
                    <h3 class="text-sm font-extrabold text-slate-800 leading-tight group-hover:text-[#005b96] transition-colors pr-2"><?php echo e($item->omschrijving); ?></h3>
                </div>
                ...
                <div class="shrink-0 flex flex-col sm:flex-row lg:flex-col items-end sm:items-center lg:items-end gap-2 border-l border-slate-100 pl-4 py-1">
                    <input type="number" id="qty-main-<?php echo e($item->id); ?>" min="1" max="<?php echo e($item->beschikbaar > 0 ? $item->beschikbaar : 1); ?>" value="1" <?php echo e($item->beschikbaar == 0 ? 'disabled' : ''); ?> class="w-14 h-8 bg-slate-50 border border-slate-200 text-slate-800 rounded-lg focus:outline-none focus:border-[#005b96] text-center font-bold text-xs">
                    <button onclick="addToCart(<?php echo e($item->id); ?>, '<?php echo e(addslashes($item->omschrijving)); ?>', '<?php echo e($item->artikelnummer); ?>', <?php echo e($item->beschikbaar); ?>, 'main')" <?php echo e($item->beschikbaar == 0 ? 'disabled' : ''); ?> class="w-14 h-8 bg-slate-900 hover:bg-[#005b96] disabled:bg-slate-200 disabled:text-slate-400 text-white rounded-lg transition-colors flex items-center justify-center">
